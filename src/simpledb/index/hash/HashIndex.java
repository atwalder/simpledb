...
public class HashIndex implements Index {
	...
	public void beforeFirst(Constant searchkey) {
		close(); //close all other buckets in use
		this.searchkey = searchkey;
		int bucket = searchkey.hashCode() % NUM_BUCKETS; //bucket code? aka extend upon
		String tblname = idxname + bucket; //name the file something that makes sense?
		TableInfo ti = new TableInfo(tblname, sch); //this would be the global file????
	 ...
	 */
	public void insert(Constant val, RID rid) { //need to check if local index is greater than global index
		beforeFirst(val);
		ts.insert();
	 ...
	 */
	public void delete(Constant val, RID rid) { //hash wont shrink
		beforeFirst(val);
		while(next())
